...
COURSEBASEDIR = (
    JUPYTER_DIR + "/course/{slug}"
)  # course base dir (contains course/data dirs)
...
def setperm(path, perm, dirs_only=False):
    ...
    if dirs_only:
        ...
        dirs_only = ""
    os.system(
        "find %s %s ! -perm %s -exec chmod %s {} \\+" % (path, dirs_only, perm, perm)
    )


def setgrp(path, gid):
    """Recursively set group, don't touch if OK to preserve ctime"""
    os.system("find %s ! -group %s -exec chgrp %s {} \\+" % (path, gid, gid))
...
def assert_stat(path, mode, match="exact", missing_ok=False, mask=0o7777):
    ...
    if match == "any":
        assert stat_result & mask, "%s %o! .(any). %o (mask %o)" % (
            path,
            stat_result,
            mode,
            mask,
        ...
        assert stat_result & mask == mode, "%s %o!=%o (mask %o)" % (
            path,
            stat_result,
            mode,
            mask,
...
class Course:
    ...
    def has_datadir(self):
        return self.data.get("datadir", False)

    @property
    def coursebasedir(self):
        return Path(COURSEBASEDIR.format(slug=self.slug))

    @property
    def coursedir(self):
        return Path(COURSEDIR.format(slug=self.slug))

    @property
    def exchangedir(self):
        return Path(EXCHANGEDIR) / self.slug

    @property
    def datadir(self):
        if not self.has_datadir:
            return None
    ...
    def setup(self):
        ...
        if (self.exchangedir / self.slug / "inbound").exists():
            for dir_ in [
                self.exchangedir / self.slug / "inbound",
